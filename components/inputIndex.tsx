
function InputIndex({ newMessage, setNewMessage, handleSubmit, user }: any) {

    return (
        <form onSubmit={handleSubmit}>
            <div className="flex items-center mb-10 py-2 px-3 rounded-lg">
                {user ? (
                    <input
                        type="text"
                        onChange={(e) => setNewMessage(e.target.value)}
                        value={newMessage}
                        name="message"
                        required
                        placeholder="Message..."
                        className="block mx-4 p-2.5 w-full text-sm rounded-lg border text-gray-500 focus:ring-blue-500 outline-none"
                    />
                ) : (
                    <input
                        type="text"
                        name="message"
                        placeholder="Login to send messages..."
                        className="block mx-4 p-2.5 w-full text-sm rounded-lg border text-gray-500 focus:ring-blue-500 outline-none"
                        disabled
                    />
                )}
                <button type="submit" className="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer hover:bg-gray-100">
                    <svg className="w-6 h-6 rotate-90" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"></path></svg>
                </button>
            </div>
        </form>
    )
}

export default InputIndex;